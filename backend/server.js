...
// ── RAZORPAY ──────────────────────────────────────────────────────────────────
// ✅ FIXED: Both fallback values now use the SAME keypair from the same Razorpay account.
//    KEY_ID  and KEY_SECRET below are from the same account.
//    If you rotate keys, update BOTH values together — they must always be a matched pair.
//    The .env variables RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET take priority at runtime.
const RZP_KEY_ID     = (process.env.RAZORPAY_KEY_ID     || 'rzp_test_Soiy52I6HkMaEN').trim();
const RZP_KEY_SECRET = (process.env.RAZORPAY_KEY_SECRET || 'dlNQKRcxwDzqNd6aKqF7AyOv').trim();

console.log('🔑 Razorpay Key ID :', RZP_KEY_ID.slice(0, 22) + '...');
...
try {
  if (RZP_KEY_ID && RZP_KEY_SECRET && RZP_KEY_ID.startsWith('rzp_')) {
    ...
    console.log('✅ Razorpay initialized with key:', RZP_KEY_ID.slice(0, 18) + '...');
  } else {
    console.warn('⚠️  Razorpay keys invalid — running in simulated mode');
  }
} catch (e) {
...
// ── HEALTH ────────────────────────────────────────────────────────────────────
app.get('/',    (req, res) => res.json({ status: 'Cine Time API ✅', version: '9.1', razorpay: razorpay ? 'live' : 'simulated', keyId: RZP_KEY_ID.slice(0, 16) + '...' }));
app.get('/api', (req, res) => res.json({ status: 'Cine Time API ✅', version: '9.1', razorpay: razorpay ? 'live' : 'simulated', keyId: RZP_KEY_ID }));
...
// ── RAZORPAY ──────────────────────────────────────────────────────────────────
// ✅ CRITICAL: keyId is ALWAYS returned in every response path.
//    The frontend uses ONLY this keyId — it never has a hardcoded key.
//    This prevents the 401 Unauthorized "key mismatch" error.
app.post('/api/payment/create-order', async (req, res) => {
  try {
    const amount = parseFloat(req.body.amount);
    console.log(`💰 create-order ₹${amount} | mode=${razorpay ? 'LIVE' : 'SIMULATED'} | key=${RZP_KEY_ID.slice(0, 18)}...`);

    if (!amount || amount <= 0) {
      return res.status(400).json({ error: 'Invalid amount' });
    }

    // Simulated mode — Razorpay not initialised on server
    if (!razorpay) {
      console.log('⚠️  Razorpay not initialised — returning simulated order');
      return res.json({
        orderId:   `order_SIM_${Date.now()}`,
        amount:    Math.round(amount * 100),
        amountINR: amount,
        currency:  'INR',
        keyId:     RZP_KEY_ID,  // ← always returned so frontend knows the key
        simulated: true,
        reason:    'Razorpay keys not initialised on server',
      });
    }
    ...
    return res.json({
      ...
      keyId:     RZP_KEY_ID,  // ← always returned, matches the key that created the order
      simulated: false,
    });
    ...
    // Even on error, return a simulated order WITH keyId so the frontend can handle it
    return res.json({
      orderId:   `order_SIM_${Date.now()}`,
...
app.post('/api/payment/verify', async (req, res) => {
  try {
    ...
    // Simulated orders always pass verification
...
// ── ANALYTICS — sums Ticket + AdminBooking revenue ───────────────────────────
...
app.listen(PORT, () => {
  console.log(`🚀 Cine Time v9.1 on port ${PORT}`);
